...
class DataOrganizer():
    ...
    def __copy_files(self):
        ...
        of these files. Simulink is like a shortcut.
        I create simulink using 'ln -s' command in ubuntu,I don't like
        os.symlink() in python!!
        """
        command = "ln -s {} {}".format(src, dst)
        os.symlink(os.path.join("archive", "utils"),\
                   os.path.join(self.OUTDIR, "utils"))
        # os.symlink(os.path.join("archive", "steps"),\
        #            os.path.join(self.OUTDIR, "steps"))
